...
Handles global graph analysis for comprehensive retrieval in the UAE Legal GraphRAG system.
"""
...
class GlobalGraphRAGAgent(BaseAgent):
    """Agent responsible for global graph analysis and retrieval."""
    
    def __init__(self, neo4j_conn, azure_llm):
        super().__init__("global_graphrag_agent", neo4j_conn, azure_llm)
        self.global_rag = GlobalGraphRAG(neo4j_conn)
        
    async def handle_message(self, envelope: A2AEnvelope) -> Optional[A2AEnvelope]:
        """Handle global GraphRAG retrieval requests."""
        try:
            task_type = envelope.payload.get("task_type")
            data = envelope.payload.get("data", {})
            
            if task_type == "retrieve":
                return await self._perform_global_retrieval(data, envelope)
            else:
                return await self.send_error_response(envelope, f"Unknown task type: {task_type}")
                
        except Exception as e:
            self.logger.error(f"Global GraphRAG agent error: {e}")
            return await self.send_error_response(envelope, str(e))
    
    async def _perform_global_retrieval(self, data: Dict[str, Any], envelope: A2AEnvelope) -> A2AEnvelope:
        """Perform global graph analysis retrieval."""
        query = data.get("query", "")
        max_results = data.get("max_results", 10)
        
        self.logger.info(f"Performing global GraphRAG retrieval for query: {query}")
        
        try:
            # Execute global GraphRAG retrieval
            rag_result = await self.global_rag.retrieve(query, max_results)
            ...
            self.logger.error(f"Global retrieval error: {e}")
            return await self.send_error_response(envelope, str(e))
